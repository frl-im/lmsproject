<div style="font-family: sans-serif; padding: 2rem;">
    ...
    <h1 style="margin-top: 1rem;">Edit Module di Course: <span style="color: #007bff;">{{ $course->title }}</span></h1>
    ...
    <form action="{{ route('admin.courses.modules.update', [$course, $module]) }}" method="POST">
        ...
        <div style="margin-bottom: 1rem;">
            ...
            <input type="text" id="title" name="title" value="{{ old('title', $module->title) }}" style="width: 100%; padding: 0.5rem;" required>
        </div>
        <div style="margin-bottom: 1rem;">
            <label for="order" style="display: block; margin-bottom: 0.5rem;">Nomor Urut</label>
            <input type="number" id="order" name="order" value="{{ old('order', $module->order) }}" style="width: 100%; padding: 0.5rem;" required>
        </div>
        <div>
